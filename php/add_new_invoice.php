<?php
  ...
  function addSale() {
    ...
    if ($con) {
        // Sanitize inputs 
        $customer_name = strtoupper(mysqli_real_escape_string($con, $_GET['customers_name'])); 
        $contact_number = mysqli_real_escape_string($con, $_GET['customers_contact_number']); 

        $query = "INSERT INTO sales (CUSTOMER_ID, INVOICE_NUMBER, MEDICINE_NAME, BATCH_ID, EXPIRY_DATE, QUANTITY, MRP, DISCOUNT, TOTAL) 
                  VALUES($customer_id, $invoice_number, '$medicine_name', '$batch_id', '$expiry_date', $quantity, $mrp, $discount, $total)";
        ...
        if (!$result) {
            ...
            echo "Inserted sale successfully.";
        }
    }
}
